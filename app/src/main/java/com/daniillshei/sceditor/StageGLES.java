...
public class StageGLES implements GLSurfaceView.Renderer, Stage {
    ...
    private static StageGLES INSTANCE;
    ...
    private StageGLES() {
        ...
        StageGLES.STAGE_COUNT++;
    }

    public static StageGLES getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new StageGLES();
        }
...
//            this.gradientTexture = GLImage.createWithFormat(256, 2, true, ImageFilter.LINEAR, GLConstants.GL_LUMINANCE_ALPHA, GLConstants.GL_UNSIGNED_BYTE, imageBuffer, null, null,this);
//        }
    ...
    public GLTexture createGLTexture(SWFTexture texture, Path directory) {
...
//            GLES2RendererContext rendererContext = new GLES2RendererContext();
            ...
            KhronosTextureLoaders.registerLoader(new ExtensionKhronosTextureLoader(rendererContext));
            GLImage.khronosTextureLoader = KhronosTextureLoaders.getLoader();

            AssetManager assetManager1 = new AssetManager(new GLShaderLoader(rendererContext));

            setAssetManager(assetManager1);
            setGlContext(rendererContext);
